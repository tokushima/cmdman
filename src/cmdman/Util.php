...
class Util{
	...
	private static function parse_filename(string $filename): string{
		...
		return str_ends_with($filename,'/') ? substr($filename,0,-1) : $filename;
	}
	/**
	 * 絶対パスを返す
	 */
	public static function path_absolute(?string $a, ?string $b): string{
		...
		if(preg_match("/^[\w]+\:\/\/[^\/]+/",$a,$h)){
			...
			if(!str_ends_with($a,'/')) $b = (str_starts_with($b,'./')) ? '.'.$b : (($b[0] !== '.' && $b[0] !== '/') ? '../'.$b : $b);
		...
		for($i=0;$i<count($al)-substr_count($b,'../');$i++){
			if($al[$i] !== '.' && $al[$i] !== '..') $d .= $al[$i].'/';
		}
		...
		$d = (!empty($d) && $d[0] !== '/' && !str_starts_with($d,'#T#') && !str_contains($d,'#W#')) ? '/'.$d : $d;
	...
	/**
	 * 待機を要求して終了する (repeatコマンドが待機後に再実行)
	 */
